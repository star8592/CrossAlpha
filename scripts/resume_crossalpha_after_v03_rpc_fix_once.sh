#!/usr/bin/env bash
# Resume the complete research-stack acceptance after the known State V0.3 D2
# preflight failure. This script is intentionally narrow: it may skip the full
# pytest suite only when the immediately previous grand-finalizer summary proves
# that D2 was the sole failure and no V0.3/V0.4/Outcome freeze was created.
# It runs targeted tests for every file changed by the D2 fix, then resumes the
# gated acceptance from V0.3 onward. It always exits 0; FINAL_RESULT is authoritative.
...
LOG="/tmp/crossalpha_research_stack_resume_v03_${STAMP}.log"
LATEST="/tmp/crossalpha_research_stack_resume_v03_latest.log"
SUMMARY="/tmp/crossalpha_research_stack_resume_v03_summary.json"
PREVIOUS_SUMMARY="/tmp/crossalpha_research_stack_finalize_summary.json"
ln -sfn "$LOG" "$LATEST"
exec > >(tee -a "$LOG") 2>&1
...
wait_new_oneshots_idle() {
  ...
  for attempt in $(seq 1 720); do
    ...
    if [[ $busy -eq 0 ]]; then
      echo "All CrossAlpha oneshot services are idle."
      return 0
    fi
    sleep 1
  done
  echo "Timed out waiting for CrossAlpha oneshot services to become idle." >&2
  return 1
}
...
echo "CrossAlpha D2 resume finalizer"
echo "repo=$REPO_DIR"
...
# R0. Prove this narrow resume path is legitimate. Never skip a full suite based
# on memory or assumption; the previous machine-readable grand-finalizer result
# must prove D2 was the sole failure before any newer protocol freeze existed.
run_critical "R0. prove previous grand finalizer stopped only at D2" python - "$PREVIOUS_SUMMARY" <<'PY'
import json, sys
from pathlib import Path
path = Path(sys.argv[1])
if not path.exists():
    raise SystemExit("previous grand-finalizer summary missing; use the grand finalizer instead")
row = json.loads(path.read_text(encoding="utf-8"))
expected = ["D2. preflight + freeze State V0.3::1"]
assert row.get("result") == "FAILED", row
assert row.get("failures") == expected, row
assert row.get("A_hash_unchanged") is True, row
assert row.get("AB_hash_unchanged") is True, row
assert row.get("V03_hash_final") == "MISSING", row
assert row.get("V04_hash_final") == "MISSING", row
assert row.get("OUTCOME_hash_final") == "MISSING", row
print(json.dumps({"resume_precondition_ok": True, "previous_failures": expected}, indent=2))
PY

if phase_ok; then
  if [[ "$A_HASH_BASELINE" == MISSING || "$AB_HASH_BASELINE" == MISSING || "$V02_HASH_BASELINE" == MISSING ]]; then
    FAILURES+=("R0b. predecessor freeze missing::1")
  fi
  if [[ "$V03_HASH_START" != MISSING || "$V04_HASH_START" != MISSING || "$OUTCOME_HASH_START" != MISSING ]]; then
    FAILURES+=("R0c. resume requires V03/V04/Outcome still unfrozen::1")
  fi
fi
...
if phase_ok; then
  ...
  run_critical "R3. targeted State V0.3 regression suite" pytest -q \
    ...
    tests/test_resume_finalizer_contract.py
fi

# R2. Re-prove immutable predecessors before any new freeze.
if phase_ok; then
  run_critical "R4. Frozen B3 integrity" python scripts/check_free_paper_integrity.py
  run_critical "R5. State A/B V0.1 strict integrity" python scripts/check_state_ab_integrity.py
  run_critical "R6. State V0.2 config consistency" crossalpha-state-v02-config-check
  run_critical "R7. State V0.2 strict integrity" crossalpha-state-v02-integrity
fi
V02_HASH_EXPECTED="$(sha_or_missing "$V02_FREEZE")"

# D. Resume exactly where the previous run stopped.
if phase_ok; then
  run_critical "D1. State V0.3 config consistency" crossalpha-state-v03-config-check
fi
if phase_ok && [[ ! -f "$V03_FREEZE" ]]; then
  run_critical "D2. capability-probed preflight + freeze State V0.3" crossalpha-state-v03-freeze
fi
if phase_ok; then
...
# E. State V0.4 multi-venue mechanics.
if phase_ok; then
...
# F. Deterministic Outcome Linkage.
if phase_ok; then
...
# G. Automation and race-free final audit.
if phase_ok; then
...
run_warning "I2. V0.3 recent service log" bash -lc 'journalctl --user -u crossalpha-state-v03.service -n 60 --no-pager || true'
run_warning "I3. V0.4 recent service log" bash -lc 'journalctl --user -u crossalpha-state-v04.service -n 60 --no-pager || true'
run_warning "I4. Outcome recent service log" bash -lc 'journalctl --user -u crossalpha-outcome-linkage.service -n 60 --no-pager || true'
...
if [[ "$V02_HASH_FINAL" != "$V02_HASH_BASELINE" ]]; then FAILURES+=("J3. V02 freeze hash changed::1"); fi
...
export CROSSALPHA_RESUME_FAILURES="$(printf '%s\n' "${FAILURES[@]-}")"
export CROSSALPHA_RESUME_WARNINGS="$(printf '%s\n' "${WARNINGS[@]-}")"
export CROSSALPHA_RESUME_LOG="$LOG"
...
import json, os, sys
from pathlib import Path
failures = [x for x in os.environ.get("CROSSALPHA_RESUME_FAILURES", "").splitlines() if x]
warnings = [x for x in os.environ.get("CROSSALPHA_RESUME_WARNINGS", "").splitlines() if x]
payload = {
    "result": "PASS" if not failures else "FAILED",
    "resume_mode": "AFTER_V03_D2_ONLY",
    "full_pytest_repeated": False,
    "failures": failures,
    "warnings": warnings,
    ...
    "V02_hash_unchanged": os.environ.get("CROSSALPHA_V02_HASH_BASELINE") == os.environ.get("CROSSALPHA_V02_HASH_FINAL"),
    ...
    "log": os.environ.get("CROSSALPHA_RESUME_LOG"),
}
...
echo
echo "=================================================="
echo "CROSSALPHA D2 RESUME SUMMARY"
echo "=================================================="
