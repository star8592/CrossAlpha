...
The default unified daemon owns only Observatory + bounded materializer.
State V0.3/V0.4 are opt-in and require production native integrity gates.
...
printf 'observatory_active=%s\nobservatory_enabled=%s\nmaterializer_active=%s\nmaterializer_enabled=%s\ndata_root=%s\n' \
  "$obs_active" "$obs_enabled" "$mat_active" "$mat_enabled" "$DATA_ROOT" > "$META"
...
PY
  if (( AFTER >= BEFORE + 3 )) && [[ "$DAEMON_OK" == true && "$MATERIALIZER_OK" == true ]]; then
    break
  fi
  sleep 2
done
