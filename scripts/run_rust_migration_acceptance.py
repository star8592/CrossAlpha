...
def _production_state_integrity(data_root: Path, env: dict[str, str]) -> dict[str, Any]:
    ...
    output["ok"] = all(output[version]["ok"] for version in versions)
    return output
...
def _post_cutover_gate(data_root: Path, min_soak_seconds: int) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "ok": False,
            "reason": f"invalid daemon health JSON: {type(exc).__name__}",
        }
    if status.get("status") != "running":
        return {
            "ok": False,
            "reason": f"daemon status is {status.get('status')!r}",
            "daemon": status,
        }
    try:
        ...
        return {
            "ok": False,
            "reason": f"invalid daemon checked_at: {type(exc).__name__}",
        }
    ...
    all_running_components_healthy = bool(components) and all(
        row["ok"] for row in health.values()
    )
    full_component_set = {
        "Observatory",
        "Materializer",
...
def main() -> int:
    ...
    gates: list[Gate] = [
        ...
        Gate(
            "manifest_parity",
            (str(REPO_ROOT / "target/debug/crossalpha-rs"), "manifest-parity", str(data_root)),
            "real_data",
        ),
        Gate(
            "observatory_health_parity",
            (py, "scripts/verify_rust_observatory_health_parity.py", "--data-root", str(data_root)),
            "real_data",
        ),
        Gate(
            "observatory_live_health_parity",
            (py, "scripts/verify_rust_observatory_live_health_parity.py", "--data-root", str(data_root)),
            "real_data",
        ),
        Gate(
            "canonical_parser_parity",
            (py, "scripts/verify_rust_canonical_parser_parity.py", "--data-root", str(data_root)),
            "real_data",
        ),
        Gate(
            "canonical_parquet_parity",
            (py, "scripts/verify_rust_canonical_parquet_parity.py", "--data-root", str(data_root)),
            "real_data",
        ),
        Gate(
            "canonical_materializer_parity",
            (py, "scripts/verify_rust_canonical_materializer.py", "--data-root", str(data_root)),
            "real_data",
        ),
        Gate(
            "feature_parity",
    ...
    if not args.skip_live:
        gates.extend(
            [
                Gate(
                    "observatory_collectors_live",
                    (py, "scripts/verify_rust_observatory_collectors.py"),
                    "live",
                ),
                Gate(
                    "state_v02_preflight_live",
                    (
                ...
                Gate(
                    "state_v03_preflight_live",
                    (py, "scripts/verify_rust_state_v03_preflight_parity.py"),
                    "live",
                ),
                Gate(
    ...
    daemon_cutover_allowed = (
        gates_green and lock_gate["ok"] and clean_gate["ok"] and not args.skip_live
    )
    ...
    obs_retire = (
        daemon_cutover_allowed
        and post_cutover.get("ok") is True
        and "Observatory" in running
    )
    materializer_retire = (
        daemon_cutover_allowed
        and post_cutover.get("ok") is True
        and "Materializer" in running
    )
    state_v02_retire = (
        obs_retire
        and production_state.get("v02", {}).get("ok") is True
        and "StateV02" in running
    )
    state_v03_retire = (
        obs_retire
        and production_state.get("v03", {}).get("ok") is True
        and "StateV03" in running
    )
    state_v04_retire = (
        obs_retire
