use crate::baseline::{BaselineConfig, RISK_ASSETS, apply_constraints, rolling_compound, scale_to_target_vol};
...
pub const ALL_ASSETS: [&str; 9] = [
    "US_EQUITY", "US_GROWTH", "GOLD", "SILVER", "COPPER", "WTI", "BTC", "ETH", "CASH",
];
...
pub fn build_daily_panel(rows: &[AssetReturnRow], start: NaiveDate, end: NaiveDate) -> Result<DailyPanel> {
    if end <= start {
    ...
    for asset in RISK_ASSETS {
        let Some(asset_rows) = by_asset.get_mut(asset) else { continue; };
        asset_rows.sort_by_key(|row| row.date);
        let mut seen = BTreeSet::new();
        let inception = asset_rows.first().map(|row| row.date.date_naive()).unwrap();
        for row in asset_rows.iter() {
            let day = row.date.date_naive();
            if !seen.insert(day) { bail!("duplicate research dates for {asset}"); }
            if let Some(index) = position.get(&day).copied() {
                if let Some(value) = row.daily_return { returns.get_mut(asset).unwrap()[index] = value; }
            }
        }
        for (index, day) in dates.iter().copied().enumerate() {
            if day >= inception {
                available.get_mut(asset).unwrap()[index] = true;
                if !returns[asset][index].is_finite() { returns.get_mut(asset).unwrap()[index] = 0.0; }
            }
        }
    }

    let cash_rows = by_asset.get_mut("CASH").context("CASH series missing from free Core returns")?;
    cash_rows.sort_by_key(|row| row.date);
    let mut cash_by_day = BTreeMap::<NaiveDate, Option<f64>>::new();
    for row in cash_rows.iter() { cash_by_day.insert(row.date.date_naive(), row.daily_return); }
    let mut last_cash = 0.0;
    for (index, day) in dates.iter().copied().enumerate() {
        if let Some(Some(value)) = cash_by_day.get(&day) { last_cash = *value; }
        returns.get_mut("CASH").unwrap()[index] = last_cash;
    }
    Ok(DailyPanel { dates, returns, available })
}

pub fn compute_frozen_b3_target(panel: &DailyPanel, signal_date: NaiveDate) -> Result<BTreeMap<String, f64>> {
    let index = panel.dates.iter().position(|day| *day == signal_date)
        .with_context(|| format!("signal date missing from frozen B3 inputs: {signal_date}"))?;
    if panel.dates.last().copied().is_some_and(|day| day > signal_date) {
    ...
    for asset in RISK_ASSETS {
        ...
        let trend = rolling_compound(&series[..=index], cfg.trend_window_days)?;
        let eligible = panel.available.get(asset).and_then(|values| values.get(index)).copied().unwrap_or(false)
            && trend[index].is_some_and(|value| value > 0.0);
    ...
    let total = ALL_ASSETS.iter().map(|asset| weights.get(*asset).copied().unwrap_or(0.0)).sum::<f64>();
    if (total - 1.0).abs() > 1e-10 { bail!("frozen B3 snapshot weights do not sum to one"); }
    if weights.values().any(|value| *value < -1e-12) { bail!("frozen B3 snapshot contains negative weight"); }
    Ok(weights)
}

pub fn apply_shadow_multiplier(a_weights: &BTreeMap<String, f64>, multiplier: f64) -> Result<BTreeMap<String, f64>> {
    if ![1.0, 0.75, 0.50].contains(&multiplier) { bail!("State multiplier outside frozen set"); }
