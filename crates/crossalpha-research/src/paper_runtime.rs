...
pub fn create_snapshot(
    ...
    if effective != now.date_naive() {
        bail!("prospective snapshots cannot be backfilled or future-dated");
    }
    if effective.weekday().num_days_from_monday() != 0 {
        bail!("frozen B3 paper snapshots are allowed only on Monday UTC");
    }
    let first_eligible = parse_date(freeze.get("first_eligible_effective_date"))?;
    if effective < first_eligible {
        bail!("effective date predates first prospective eligible date");
    }
    ...
    if !input_path.exists() {
        bail!(
            "paper snapshot requires exact point-in-time Core range: {}",
            input_path.display()
        );
    }
    ...
    if panel.dates.last().copied() != Some(signal) {
        bail!("paper snapshot input must end exactly on signal date {signal}");
    }
    ...
    for asset in ALL_ASSETS {
        let last = rows
            .iter()
            .filter(|row| row.economic_asset == asset && row.daily_return.is_some())
            .map(|row| row.date)
            .max();
        last_return_dates.insert(
            asset.to_owned(),
            last.map(|value| Value::String(value.to_rfc3339()))
                .unwrap_or(Value::Null),
        );
    }
    let risk_gross = RISK_ASSETS
        .iter()
...
pub fn mark_forward(data_root: &Path, end: NaiveDate, now: DateTime<Utc>) -> Result<Value> {
    require_binding(data_root)?;
    let freeze = load_legacy_freeze(data_root)?;
    if end > now.date_naive() {
        bail!("paper marks cannot use a future end date");
    }
    let start = NaiveDate::parse_from_str(HISTORICAL_START, "%Y-%m-%d")?;
    let input_path = returns_path(data_root, start, end);
    if !input_path.exists() {
        bail!(
            "paper mark requires Core returns ending {end}: {}",
            input_path.display()
        );
    }
    let rows = read_asset_returns(&input_path)?;
    ...
    if snapshots.is_empty() {
        return Ok(
            json!({"paper_protocol":PAPER_PROTOCOL,"status":"no_snapshots","created_marks":0,"skipped_existing_marks":0,"end_exclusive":end}),
        );
    }
    let mut snapshot_by_date = BTreeMap::<NaiveDate, Value>::new();
    for row in snapshots {
        snapshot_by_date.insert(parse_date(row.get("effective_date"))?, row);
    }
    ...
    let existing_dates: BTreeSet<NaiveDate> = existing
        ...
        .collect();
    let mut previous_equity = existing
        .last()
        .and_then(|row| row.get("equity_after"))
        .and_then(Value::as_f64)
        .unwrap_or(1.0);
    let mut previous_cash_equity = existing
        .last()
        .and_then(|row| row.get("cash_equity_after"))
        .and_then(Value::as_f64)
        .unwrap_or(1.0);
    let first_effective = *snapshot_by_date
        .keys()
        .next()
        .context("snapshot date missing")?;
    let mut current_weights = ALL_ASSETS
        .iter()
        .map(|asset| {
            (
                (*asset).to_owned(),
                if *asset == "CASH" { 1.0 } else { 0.0 },
            )
        })
        .collect::<BTreeMap<_, _>>();
    let mut active_snapshot: Option<Value> = None;
    if let Some(last_existing) = existing_dates.iter().max().copied() {
        for (day, snapshot) in &snapshot_by_date {
            if *day <= last_existing {
                active_snapshot = Some(snapshot.clone());
                current_weights = weights_from(snapshot)?;
            }
        }
    }
    let by_date = panel
    ...
    for day in panel.dates.iter().copied() {
        if day < first_effective || day >= end {
            continue;
        }
        if existing_dates.contains(&day) {
            skipped += 1;
            continue;
        }
        let mut turnover = 0.0;
        if let Some(snapshot) = snapshot_by_date.get(&day) {
            let new_weights = weights_from(snapshot)?;
            turnover = ALL_ASSETS
                .iter()
                .map(|asset| {
                    (new_weights.get(*asset).copied().unwrap_or(0.0)
                        - current_weights.get(*asset).copied().unwrap_or(0.0))
                    .abs()
                })
                .sum::<f64>()
                * 0.5;
            ...
            if let Some((_, snapshot)) = snapshot_by_date.range(..=day).next_back() {
                active_snapshot = Some(snapshot.clone());
                current_weights = weights_from(snapshot)?;
            } else {
                continue;
        ...
        let index = by_date[&day];
        let gross = ALL_ASSETS
            .iter()
            .map(|asset| {
                current_weights.get(*asset).copied().unwrap_or(0.0)
                    * panel
        ...
        let net = gross - cost;
        let cash = panel.returns["CASH"][index];
        previous_equity *= 1.0 + net;
        previous_cash_equity *= 1.0 + cash;
        ...
        seal_in_place(&mut payload)?;
        write_immutable(&mark_path(data_root, day), &payload)?;
        created += 1;
    }
    Ok(
        json!({"paper_protocol":PAPER_PROTOCOL,"status":"marked","created_marks":created,"skipped_existing_marks":skipped,"end_exclusive":end,"latest_equity":previous_equity,"latest_cash_equity":previous_cash_equity}),
    )
}

pub fn integrity(data_root: &Path) -> Result<Value> {
    let freeze = load_legacy_freeze(data_root)?;
    let binding = verify_runtime_binding_file(&binding_path(data_root))?;
    let snapshots = load_records(&paper_root(data_root).join("snapshots"), "effective_date=")?;
    let marks = load_records(&paper_root(data_root).join("marks"), "date=")?;
    let records_ok = snapshots
        .iter()
        .chain(marks.iter())
        .all(|row| verify_seal(row).unwrap_or(false));
    let freeze_links = snapshots
        .iter()
        .chain(marks.iter())
        .all(|row| row.get("freeze_record_sha256") == freeze.get("record_sha256"));
    Ok(
        json!({"protocol":PAPER_PROTOCOL,"ok":binding&&records_ok&&freeze_links,"runtime_binding_ok":binding,"freeze_seal":true,"record_seals":records_ok,"freeze_links":freeze_links,"snapshot_count":snapshots.len(),"mark_count":marks.len(),"python_runtime_required":false}),
    )
}

fn snapshot_path(root: &Path, date: NaiveDate) -> PathBuf {
    paper_root(root)
        .join("snapshots")
...
fn mark_path(root: &Path, date: NaiveDate) -> PathBuf {
    paper_root(root)
        .join("marks")
        .join(format!("year={:04}", date.year()))
        .join(format!("month={:02}", date.month()))
...
fn weights_from(value: &Value) -> Result<BTreeMap<String, f64>> {
    let object = value
        .get("weights")
        .and_then(Value::as_object)
        .context("snapshot weights missing")?;
    Ok(ALL_ASSETS
        .iter()
        .map(|asset| {
            (
...
fn read_verified(path: &Path) -> Result<Value> {
    ...
    if !verify_seal(&value)? {
        bail!(
            "immutable paper record failed seal verification: {}",
            path.display()
        );
    }
    Ok(value)
...
fn require_binding(root: &Path) -> Result<()> {
    if !verify_runtime_binding_file(&binding_path(root))? {
        bail!("Paper native runtime binding missing, invalid, or stale");
    }
    Ok(())
}
...
fn native_source_hashes(root: &Path) -> Result<BTreeMap<String, String>> {
    let files = [
        ...
        ("data", "crates/crossalpha-data/src/free_core.rs"),
        ("returns", "crates/crossalpha-data/src/free_returns.rs"),
        ("research_cargo", "crates/crossalpha-research/Cargo.toml"),
        ("baseline", "crates/crossalpha-research/src/baseline.rs"),
        ("paper_kernel", "crates/crossalpha-research/src/paper.rs"),
        (
            "paper_runtime",
            "crates/crossalpha-research/src/paper_runtime.rs",
        ),
        ("paper_cli", "crates/crossalpha-cli/src/bin/paper.rs"),
    ];
    let mut result = BTreeMap::new();
    for (name, relative) in files {
        let path = root.join(relative);
        if !path.exists() {
            bail!("paper binding source missing: {}", path.display());
        }
